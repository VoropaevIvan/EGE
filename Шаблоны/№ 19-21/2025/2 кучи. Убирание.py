...
def f(x1, x2, c, win):
    if x1 + x2 <= 30:  # Указываем условие окончания игры
        return c in win
    if c > max(win):
        return 0
    # Записываем все возможные ходы
    moves = [f(x1 - 1, x2, c + 1, win), f(x1, x2 - 1, c + 1, win),
             f(ceil(x1/2), x2, c + 1, win), f(x1, ceil(x2/2), c + 1, win)]
    if c % 2 != max(win) % 2:
...
for s in range(13, 1000 + 1):  # Перебираем стартовое количество камней в куче
    if f(18, s, 0, [2]) == 1:
        print('№ 19:', s)
